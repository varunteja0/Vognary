...
- PostgreSQL migrations through `0054_recovery_commitment_context` are not applied.
...
11. Query `schema_migrations` and verify the last row is `0054_recovery_commitment_context`.
...
- `capabilities.schema.status` is `ready`, and `capabilities.schema.applied` ends at `0054_recovery_commitment_context`.
...
- Feature migrations are `READY` through `0054_recovery_commitment_context`.
