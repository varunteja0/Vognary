...
- **WP-2.1 (M, choices present) One-screen onboarding.** Signed-out /app and post-signup: three buttons only — **Connect Gmail** (primary), **Paste receipts**, **See a sample audit**. Nothing else above the fold. DoD: zero mentions of CSV/statement/API on the screen.
  - **Assessed 2026-07-19:** signed-out `/app` already has the clean three-choice (guest client). Post-signup, all three choices now exist — Connect Gmail (IntegrationCommandCenter), Paste receipts (FirstSuccessPanel), and **See a sample audit** (added with WP-2.2). The empty onboarding is free of CSV/statement/API mentions (advanced file import lives in the Data-section `AdvancedImportPanel`, not first-run). Remaining: collapsing the richer post-signup onboarding (checklist + disabled action buttons) into the strict "three-buttons-only" minimalist layout is a design-judgment refactor, not missing capability.
...
- **WP-5.5 (M, DoD met) Mobile + PWA.** Bottom-nav parity for new IA; install prompt after first proven ledger; icon/splash polish.
  - **Assessed 2026-07-19:** `manifest.ts` is production-grade — standalone display, theme/background `#0b0c0f`, finance categories, and three icons including a **maskable** 512 (`public/pwa/icon-{192,512,maskable-512}.png` all present, `brand:check` green). The install prompt (after a proven ledger) and Home/Subscriptions/Connect bottom-nav parity are browser-proved in `signed-in-first-value.spec.ts`. Only marginal item outstanding: iOS per-device `apple-touch-startup-image` splash screens (a nice-to-have, not required for a quality installable PWA).
...
- **WP-6.3 (S, partial) Proof-chip everywhere.** Every ₹ figure in the product carries a tappable chip showing the exact evidence that produced it (proof-graph queries exist). This is the trust moat made visible.
  - **Home slice shipped 2026-07-18:** the aggregate figures with no single detail sheet ("Due in 30 days", "Verified savings") now carry a `ProofDisclosure` chip revealing the exact composing evidence; per-item figures already trace via the WP-1.3 detail sheet. Browser-proved desktop+mobile. Broader Subscriptions/Review sweep is the remaining slice.
