...
| A Paid proof | ACTIVE / INSTRUMENTS READY | First beachhead is India-registered, recently funded 20–100-person AI-native companies with a named finance owner and buyer-confirmed ≥₹8 lakh/month controllable exposure. CRM schema, 20 sourced public-company rows, behavioral discovery, founder-delivered offer, working-session packet, and proposal-based T0–T4 ladder are ready. Qualified/contacted/conversations/offers/payments remain 0/0/0/0/0; sourced rows and historical Autopilot outreach do not count. |
...
| C Production min | ACTIVE / EXTERNAL BLOCK | Product SHA `8286b04` is pushed and Vercel-deployed; live health is 200 and Control remains unenrolled/fail-closed on verified schema `0056`. Local CI-equivalent gates pass. GitHub CI/backup jobs cannot start because the account has failed payments or reached its Actions spending limit. A bounded `apply-control-0057` workflow requires an exact-head successful `pre-0057` encrypted backup run, exact 0056 head/checksum, migration lock, exact 0057 checksum, unchanged Recovery counts, six Control tables/triggers, and second-run refusal. Production migration, pilot UUIDs, real automatic receipts, reminder delivery, retention attestation, and first ICP completion remain unproven. |
