...
test("the real migration runner installs and records the Recovery receipt inbox on a fresh database", {
  ...
  await withDisposableDatabase("recovery_fresh", async (connectionString) => {
    ...
    assert.equal(result.applied.at(-1)?.id, "0053_phase_a_receipt_activation");
    ...
    try {
      ...
      assert.equal(migrations.rows.at(-1)?.id, "0053_phase_a_receipt_activation");
      assert.equal(migrations.rows.length, 53);
...
test("pg_dump/pg_restore preserves Recovery and audit counts through 0053", {
  skip: databaseConfigured ? false : "DATABASE_URL is required for PostgreSQL integration tests.",
}, async () => {
