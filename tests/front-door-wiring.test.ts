...
test("the landing selects the proven entry path without demo or instant-audit surfaces", () => {
  ...
  assert.match(landingSource, /const primaryLabel = "Find my recurring spend";/);
...
test("static layout metadata remains readiness-neutral", () => {
  ...
  assert.match(metadataSource, /Vognary - Know what’s renewing before you pay/);
  assert.match(metadataSource, /helps solo founders review software renewals from submitted evidence and decide what needs attention/);
