...
test("canonical product entry keeps forwarding unavailable without seeded data", async ({ page }) => {
  ...
  await page.goto("/");
  await expect(page.getByRole("heading", { level: 1, name: "Know what your company is committed to pay next — and what deserves attention before the card fires." })).toBeVisible();
