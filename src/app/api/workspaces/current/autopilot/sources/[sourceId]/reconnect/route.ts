...
import { reconnectRecoverySource } from "@/lib/server/recovery-autopilot-store";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  return runRecoveryRoute(request, {
    namespace: "recovery-autopilot-source-reconnect",
    limit: 20,
    windowMs: 60 * 60_000,
    ...
    const result = await reconnectRecoverySource({
