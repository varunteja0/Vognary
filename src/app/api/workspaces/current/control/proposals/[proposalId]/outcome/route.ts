import { normalizeControlOutcomeObservationRequest } from "@/lib/commitment-control/contracts";
...
import { recordCommitmentControlOutcomeObservation } from "@/lib/server/commitment-control-store";
...
export async function POST(request: Request, context: { params: Promise<{ proposalId: string }> }) {
  ...
  return runRecoveryRoute(request, {
    namespace: "commitment-control-outcome-write",
    limit: 120,
    windowMs: 60 * 60_000,
    ...
    const body = await readCommitmentControlRequest(request, normalizeControlOutcomeObservationRequest);
    const result = await recordCommitmentControlOutcomeObservation({
    ...
    const attentionProjection = await refreshControlAttentionAfterMutation({
      workspaceId: session.workspaceId,
      requestId,
      routePath: "/api/workspaces/current/control/proposals/[proposalId]/outcome",
    });
