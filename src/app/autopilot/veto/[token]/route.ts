...
const publicVetoScript = [
  ...
  '  if (!/^\\/autopilot\\/veto\\/[^/]+$/.test(path)) { status.textContent = "This veto link is invalid."; return; }',
  ...
  '      status.textContent = response.status === 429 ? "Too many veto attempts. No new attempt was accepted; wait and try again." : "The veto result could not be confirmed. Retrying is safe; an already-recorded veto will not be applied twice.";',
  "      return;",
  "    }",
...
const cspHash = (value: string) => createHash("sha256").update(value).digest("base64");
const publicVetoCsp = [
  "default-src 'none'",
  `script-src 'sha256-${cspHash(publicVetoScript)}'`,
  `style-src 'sha256-${cspHash(publicVetoStyle)}'`,
  "connect-src 'self'",
  "base-uri 'none'",
  "form-action 'none'",
...
export async function GET() {
  return new Response(publicVetoPageHtml(), {
    ...
    headers: {
      ...
      "content-security-policy": publicVetoCsp,
...
<body>
  ...
  <script>${publicVetoScript}</script>
</body>
</html>`;
}
